...
describe("UserSearch", () => {
  ...
  describe("유저 선택", () => {
    ...
    it("인원을 변경한 후 유저 선택 시 writeLog에 변경된 companion이 전달된다.", () => {
      fireEvent.click(screen.getByTestId("icon-increase")); // companion → 2
      fireEvent.click(screen.getByText("홍길동"));
      expect(writeLog).toHaveBeenCalledWith(mockUser.id, 2);
    });
    ...
    it("writeLog 에러 발생 시 dialog.open과 onClose가 호출되지 않는다.", () => {
